...
from robopianist.suite.tasks import piano_with_shadow_hands, hold_grabber

# RoboPianist-hold-grabber.
HOLD_GRABBER = "RoboPianist-hold_grabber-v0"
...
ALL = REPERTOIRE_150 + ETUDE_12 + DEBUG + [HOLD_GRABBER]
_ALL_DICT: Dict[str, Union[Path, str]] = {
    **_REPERTOIRE_150_DICT,
    **_ETUDE_12_DICT,
    **_DEBUG_DICT,
    HOLD_GRABBER: None,  # No MIDI file needed for hold grabber
}
...
def load(
    ...
    Args:
        environment_name: Name of the environment to load. Must be of the form
            ...
            PIG dataset MIDI file in camel case notation, or "RoboPianist-hold_grabber-v0"
            for the climbing hold task.
    ...
    # Handle hold_grabber environment
    if environment_name == HOLD_GRABBER:
        # Filter out piano-specific kwargs that hold_grabber doesn't use
        allowed_kwargs = {
            'max_episode_steps', 'disable_contact_reward', 
            'randomize_hand_positions', 'energy_penalty_coef',
            'gravity_compensation', 'primitive_fingertip_collisions',
            'reduced_action_space', 'attachment_yaw', 'forearm_dofs'
        }
        filtered_kwargs = {k: v for k, v in task_kwargs.items() if k in allowed_kwargs}
        
        return composer_utils.Environment(
            task=hold_grabber.HoldWithShadowHands(**filtered_kwargs),
            random_state=seed,
